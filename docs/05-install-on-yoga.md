...
- joske boots the **Codelinaro Debian-12 image**, then hijacks its initrd shell to install Arch
- the gist hand-waves adding tools to the initrd:

> *"unpack the initrd and add this tarball … also add some tools like mkfs.ext4 and fdisk"*

- do 1.1 on a **Debian 12 arm64 box**
  - WSL is ideal — matches the installer's glibc
...
> ⚠️ **Don't bake the rootfs into the initrd.**
> - the boot partition is a fixed ~300 MB; initrd (127 MB) + rootfs = ~900 MB won't fit (`Input/output error`)
> - keep the initrd small; carry the rootfs on a separate partition (below)

**c) Carry the rootfs on a second FAT32 partition**
...
- overwrite the boot partition's `initrd.gz` with `initrd-new.gz`
- **don't boot yet → do 1.2**
...
Sidesteps the DATA partition (the installer initrd couldn't mount it):

- enlarges the image's own FAT boot partition
- bakes rootfs + firmware into the initrd (~935 MB → RAM; boot partition ~1.5 GB)
...
Prereqs (WSL Debian arm64):

- `util-linux mtools dosfstools cpio gzip`
- the 1.1(a) bundle at `~/initrd-tools/opt`
- the rootfs tarball at `~`
- firmware at `~/qcom-firmware/` (see 1.2)
...
- Rufus DD-flash `usb-single.img`
- at the installer shell nothing needs mounting (payloads at `/root/`):
```sh
...
Use the `qcom-firmware/` from **Step 0** (`C:\qcom-firmware\`):

- it's a courier — 1.3 places it on the NVMe
- **1.1(c)** — copy onto DATA next to the rootfs
  - `cp -r /mnt/c/qcom-firmware /mnt/data/`
- **1.1-ALT** — copy to `~/qcom-firmware/` in WSL *before* running 1.1-ALT
  - ends up at `/root/qcom-firmware/`

### 1.3 — At the installer shell: partition → extract → chroot

> After 1.1 (+1.2). Flash, boot (F12), land at the initrd shell.
> - rootfs source: 1.1(c) → mount `LABEL=DATA`; 1.1-ALT → already at `/root/`
