...
| Repo | Role | Bootable? |
|---|---|---|
| **swarmarchy** (this one) | the **post-install configuration layer** — dotfiles + the `omarchy-*` command toolbox + package manifests + ordered install scripts. Runs *on top of* a base Arch system. | No |
...
2. After the base install, the ISO clones swarmarchy to `~/.local/share/omarchy` and
   runs **`install.sh`**.
3. `install.sh` exports `OMARCHY_PATH`, puts `bin/` on `PATH`, and **sources the phases
   ...
   | `packaging/` | install packages: **`base.sh`** (reads `omarchy-base.packages`), fonts, nvim, **`icons.sh`** (copies `applications/icons`), **`webapps.sh`** (installs the Chromium PWAs), tuis, hardware-specific |
...
4. **Reboot → the Sway desktop** (session entry `default/wayland-sessions/omarchy.desktop` → `Exec=sway`).
...
| `bin/` | the **`omarchy-*` command toolbox** (~280 scripts): the menu, app launchers, window/monitor toggles, screenshot/OCR, theme, lock/idle, system | every runtime command the desktop & menu call by name |
| `config/` | **user** config templates → copied to `~/.config/` | per-user dotfiles: `sway/`, `waybar/`, `foot/`, `mako/` *(via default)*, `swaylock/`, `walker/`, `btop/`, … |
| `default/` | **shipped** defaults → `~/.local/share/omarchy/default/` (the read-only layer the user configs `include`) | `sway/` defaults (bindings/appearance/input/autostart), `bash/`, `limine/`, `snapper/`, `wayland-sessions/` |
...
| `bin/omarchy-iso-make` | the entry command — `./bin/omarchy-iso-make` (output → `release/`) |
...
2. **Define the packages:** edit `install/omarchy-base.packages` (desktop + apps) and
   `install/omarchy-other.packages` (base system + drivers). Every package is one line
   with an inline `# note`. This is the master list of what the OS installs.
3. **Build `bin/`:** the `omarchy-*` scripts — your runtime commands (menu, launchers,
...
5. **Build `default/`:** the shipped defaults the user configs `include` —
   ...
   `default/wayland-sessions/omarchy.desktop` (`Exec=sway`).
...
10. **Test:** boot the ISO in a VM (`docs/05-step3-vm-testing.md`), then install on the
...
| 04 | `04-testing-sway.md` | Test *checklist*: §1 static checks (any machine) + the Sway GUI tests |
| 05 | `05-step3-vm-testing.md` | Build a VM to run those tests in — **self-contained** (setup + tests inline). Testing in a VM? Follow 05 end-to-end; you don't need to bounce to 04. |
