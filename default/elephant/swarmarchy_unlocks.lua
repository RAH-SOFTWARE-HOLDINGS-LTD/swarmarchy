--
-- Dynamic Swarmarchy Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the swarmarchy-shipped Plymouth via
-- swarmarchy-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs swarmarchy-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "swarmarchyunlocks"
NamePretty = "Swarmarchy Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/swarmarchy/themes"
  local swarmarchy_path = os.getenv("SWARMARCHY_PATH") or ""
  local default_themes_dir = swarmarchy_path .. "/themes"
  local default_preview = swarmarchy_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "swarmarchy-launch-floating-terminal-with-presentation "
                .. shell_escape("swarmarchy-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "swarmarchy-launch-floating-terminal-with-presentation "
        .. shell_escape("swarmarchy-plymouth-reset"),
    },
  }
