...
swarmarchy-pkg-add sddm libsecret gnome-keyring || error_exit
...
sudo systemctl disable swarmarchy-seamless-login.service
sudo systemctl unmask plymouth-quit-wait.service
sudo systemctl enable getty@tty1.service
sudo systemctl enable sddm.service
sudo systemctl daemon-reload

if systemctl is-enabled swarmarchy-seamless-login.service >/dev/null 2>&1; then
  echo -e "\033[31mError: swarmarchy-seamless-login.service is still enabled\033[0m" >&2
  error_exit
fi
...
sudo rm -f /etc/systemd/system/swarmarchy-seamless-login.service
