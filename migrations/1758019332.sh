...
swarmarchy-refresh-config uwsm/default
swarmarchy-refresh-config uwsm/env
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, swarmarchy-launch-editor' ~/.config/hypr/bindings.conf
fi
