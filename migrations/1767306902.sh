...
# Move user-added backgrounds from Swarmarchy theme folders to user config
SWARMARCHY_DIR="$HOME/.local/share/swarmarchy"
USER_BACKGROUNDS_DIR="$HOME/.config/swarmarchy/backgrounds"

if [[ -d $SWARMARCHY_DIR/themes ]]; then
  cd "$SWARMARCHY_DIR"

  # Get list of git-tracked background files (relative to swarmarchy dir)
...
THEMES_DIR="$HOME/.config/swarmarchy/themes"
CURRENT_THEME_LINK="$HOME/.config/swarmarchy/current/theme"
...
elif [[ -f $HOME/.config/swarmarchy/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/swarmarchy/current/theme.name")
fi

# Remove all symlinks from ~/.config/swarmarchy/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  swarmarchy-theme-set "$CURRENT_THEME_NAME"
else
