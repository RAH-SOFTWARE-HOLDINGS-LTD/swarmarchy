echo -e "Offer new Swarmarchy hotkeys\n"
...
if gum confirm "Switch to new hotkeys?"; then
  ...
  sed -i 's|source = ~/.local/share/swarmarchy/default/hypr/bindings/tiling\.conf|source = ~/.local/share/swarmarchy/default/hypr/bindings/clipboard.conf\
source = ~/.local/share/swarmarchy/default/hypr/bindings/tiling-v2.conf|' ~/.config/hypr/hyprland.conf
fi
