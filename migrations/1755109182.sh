...
# Users can still opt-in to Cloudflare DNS using: swarmarchy-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-swarmarchy-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-swarmarchy-dns.network
  ...
  echo "To use Cloudflare DNS, run: swarmarchy-setup-dns Cloudflare"
fi